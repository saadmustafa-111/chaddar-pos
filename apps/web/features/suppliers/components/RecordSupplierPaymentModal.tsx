...
interface Props {
  ...
  onSaved: (entry: SupplierLedgerEntry) => void;
}
...
export function RecordSupplierPaymentModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const entry = await suppliersApi.recordPayment(supplier.id, payload);
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <SummaryTile
            ...
            variant={outstanding > 0 ? 'highlight' : 'success'}
            helper={
              outstanding > 0
          ...
          {nothingDue && (
            <InlineInfo>
              This supplier has no outstanding balance. Nothing to record.
            </InlineInfo>
          )}
